...
export default function SecThreeThreeA() {
  const [isUndergraduateResearch, setIsUndergraduateResearch] = useState("");
  const { values, setFieldValue } = useFormikContext();

  return (
    <FieldArray name="undergraduateResearchDetails">
      {({ push, remove }) => (
        <div>
          <fieldset className="mb-4">
            <div className="flex items-center gap-4">
              <p className="mt-1 mb-2 font-medium">
                3.3.A Undergraduate research students
              </p>
              ...
              <div className="flex flex-wrap items-center gap-x-6 gap-y-2">
                {["Fill", "N/A"].map((title) => (
                  <div className="flex items-center gap-x-2" key={title}>
                    <Field
                      type="radio"
                      name="undergraduateResearchDetail"
                      value={title}
                      id={title}
                      checked={values.undergraduateResearchDetail === title}
                      onChange={() => {
                        setFieldValue("undergraduateResearchDetail", title);
                        setIsUndergraduateResearch(title);
                      }}
                    />
                    <label
                      htmlFor={title}
            ...
            <>
              <div className="overflow-x-auto">
                <table className="min-w-full bg-white border border-gray-300">
                  ...
                  <tbody>
                    {values.undergraduateResearchDetails.map((item, index) => (
                      <tr key={index}>
                        ...
                        <td>
                          <Field
                            type="text"
                            name={`undergraduateResearchDetails[${index}].dProgramme`}
                        ...
                        <td>
                          <Field
                            as="select"
                            name={`undergraduateResearchDetails[${index}].supervision`}
                            className="w-full border px-2 py-1 rounded"
                          >
                        ...
                        <td>
                          <Field
                            as="select"
                            name={`undergraduateResearchDetails[${index}].accYear`}
                            className="w-full border px-2 py-1 rounded"
                          >
                        ...
                        <td>
                          <Field
                            type="text"
                            name={`undergraduateResearchDetails[${index}].regNumber`}
                            className="w-full border px-2 py-1 rounded"
                          />
                        </td>
                        <td className="px-4 py-2 border text-center">
